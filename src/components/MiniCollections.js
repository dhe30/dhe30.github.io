...
function MiniCollections({path, title, image, description, date}) {
    ...
    return (
        <Row className="m-0 p-0 test" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
            <Col className="test pb-3 pt-5" xs="auto" style={{position:"relative",borderLeft:"3px dashed black"}}>
                    <img 
                        alt="" 
                        style={{
                            objectFit: "cover",
                    ...
                    <div className="p-0 m-0" style={{height:"16px", width:"16px", border:"3px solid black", borderRadius:"8px", backgroundColor:"white", position:"absolute", top:"110px", left:"-10px"}}></div>
            </Col>
            <Col sm='12' className="sm-12 pt-5" style={{maxWidth:"434px"}}>
                <span className="test istok-web-regular" style={{display: "block"}}>
                    ...
                    <SpecialLink show={show} path={path.toLowerCase() + `#${title}`} title={title}></SpecialLink>
                </span>
